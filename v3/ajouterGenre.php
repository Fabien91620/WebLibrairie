...
<div class="wrapper" id="wrapper">
    ...
    <section class="my_account_area pt--80 pb--55 bg--white">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="section__title text-center">
                        <h2 class="title__be--2">Ajouter <span class="color--theme">Genres</span></h2>
                    </div>
                </div>
            </div>
            <div id="tablePanier" class="table-content wnro__table table-responsive">
                <table id="tableLivre">
                    ...
                    <tr class="title-top">
                        <th></th>
                        <th class="product-thumbnail">LIBELLE</th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php $nb_liens=0; $link = connectDB(); ?>
                    <?php $genre = getAjouterGenre(); ?>
                    <?php foreach ($genre as $data):?>
                        ...
                        <tr id="idTr_<?= $nb_liens ?>">
                            ...
                            <td><textarea onclick="save(this.id, <?php echo $nb_liens ?>, true);" id="genre_<?php echo $nb_liens ?>" style="width: 1000px; background: #e5e5e5 none repeat scroll 0 0; border: medium none; border-radius: 3px; color: #333; font-size: 15px; font-weight: normal; height: 40px; padding: 0 5px 0 10px;"><?= $data['LIBELLE'] ?></textarea></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <?php
        $requetteId = $link->query("SELECT MAX(ID) as id FROM GENRE");
            ...
            if (!id_sel.is(e.target)&& id_sel.has(e.target).length === 0) {
                ...
                if (valeurDivSortie != valeurDiv || valeurDivSortie == null) {
                    if(bool == true) {
                        ...
                        $.ajax({
                            url: 'Ajax/ajaxGenreUpdate.php',
                            type: 'POST',
                            data: 'id=' + id_ligne + '&genre=' + id_genre,
                            dataType: 'html',
                            success: function (code_html, statut) {
...
<script>
    ...
    $("#btnAjouterLigne").click(function(){
        ...
        $("#tableLivre>tbody:last").append(
            ...
            "<td><textarea style='width: 1000px; background: #e5e5e5 none repeat scroll 0 0; border: medium none; border-radius: 3px; color: #333; font-size: 15px; font-weight: normal; height: 40px; padding: 0 5px 0 10px;' onclick=save1('genre_"+ nb_entrees+"','"+nb_entrees+"','"+nb_add_ligne+"'); id='genre_"+ nb_entrees+"' name='genre_"+ nb_entrees+"' type='text'></textarea></td>" +
            "</tr>" +

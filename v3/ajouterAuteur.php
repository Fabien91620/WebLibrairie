...
<div class="wrapper" id="wrapper">
    ...
    <section class="my_account_area pt--80 pb--55 bg--white">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="section__title text-center">
                        <h2 class="title__be--2">Ajouter <span class="color--theme">Écrivain</span></h2>
                    </div>
                </div>
            </div>
            <div id="tablePanier" class="table-content wnro__table table-responsive">
                <table id="tableLivre">
                    ...
                    <tr class="title-top">
                        ...
                        <th class="product-thumbnail">Prénom</th>
                        <th class="product-thumbnail">Date de naissance</th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php $nb_liens=0; $link = connectDB(); ?>
                    <?php $livres = getAjouterAuteur(); ?>
                    <?php foreach ($livres as $data):?>
                        ...
                        <tr id="idTr_<?= $nb_liens ?>">
                            ...
                            <td><textarea onclick="save(this.id, <?php echo $nb_liens ?>, true);" id="nom_<?php echo $nb_liens ?>" style="width: 200px; background: #e5e5e5 none repeat scroll 0 0; border: medium none; border-radius: 3px; color: #333; font-size: 15px; font-weight: normal; height: 40px; padding: 0 5px 0 10px;"><?= $data['NOM'] ?></textarea></td>
                            <td><textarea onclick="save(this.id, <?php echo $nb_liens ?>, true);" id="prenom_<?php echo $nb_liens ?>" style="width: 200px; background: #e5e5e5 none repeat scroll 0 0; border: medium none; border-radius: 3px; color: #333; font-size: 15px; font-weight: normal; height: 40px; padding: 0 5px 0 10px;"><?= $data['PRENOM'] ?></textarea></td>
                            <td><input onclick="save(this.id, <?php echo $nb_liens ?>, true);" id="date_<?php echo $nb_liens ?>" type="date" style="width: 200px;" value="<?= $data['DT_NAISS'] ?>"></td>
                            </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <?php
        $requetteId = $link->query("SELECT MAX(id) as id FROM ECRIVAIN");
            ...
            if (!id_sel.is(e.target)&& id_sel.has(e.target).length === 0) {
                ...
                if (valeurDivSortie != valeurDiv || valeurDivSortie == null) {
                    if(bool == true) {
                        ...
                        $.ajax({
                            url: 'Ajax/ajaxEcrivainUpdate.php',
                            type: 'POST',
                            data: 'id=' + id_ligne + '&nom=' + id_nom + '&prenom=' + id_prenom + '&date=' + id_date,
                            dataType: 'html',
                            success: function (code_html, statut) {
...
<script>
    ...
    function del(idLigne, idTr) {
        ...
        $.ajax({
            url : 'Ajax/ajaxEcrivainDelete.php',
            type : 'POST',
            data : 'id=' + id_ligne,
            dataType : 'html',
            success : function(code_html, statut){
